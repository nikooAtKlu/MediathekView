...
{
    private static final Logger LOG = LogManager.getLogger(PanelEinstellungenGeo.class);
    ...
    private void init()
    {
        final String geoText = MVConfig.get(MVConfig.Configs.SYSTEM_GEO_STANDORT);
        GeoLocations geo = GeoLocations.getFromDescription(geoText);
        if (geo == null)
        {
            try
            {
                geo = GeoLocations.valueOf(geoText);
            } catch (IllegalArgumentException illegalArgumentException)
            {
                LOG.error(String.format("Die GEO-Location \"%s\" konnte nicht geladen werden.", geoText), illegalArgumentException);
                geo = GeoLocations.GEO_DE;
            }
        }
        switch (geo)
        {
